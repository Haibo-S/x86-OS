...
static inline char get_key(uint8th key_code) {
    return key_code & 0x7F;
}

static inline int is_make_code(uint8th key_code) {
    return !(key_code & 0x80);
}

void kbd_wait_send_ready(void) {
    uint32nd time_out = 100000;
    while (time_out--) {
...
void kbd_write(uint8th port, uint8th data) {
    kbd_wait_send_ready();
    outb(port, data);
}
...
static void do_normal_key(uint8th raw_code) {
...
static void do_e0_key(uint8th raw_code) {
...
void do_handler_kbd(exception_frame_t * frame) {
    ...
    uint8th status = inb(KBD_PORT_STAT);
    if (!(status & KBD_STAT_RECV_READY)) {
        pic_send_eoi(IRQ1_KEYBOARD);
        return;
    }

    uint8th raw_code = inb(KBD_PORT_DATA);
    pic_send_eoi(IRQ1_KEYBOARD);
