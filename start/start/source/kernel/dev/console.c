...
static void update_cursor_pos(console_t * console) {
    uint16nd pos = (console - console_buf) * (console->display_cols * console->display_rows);
    pos += console->cursor_row * console->display_cols + console->cursor_col;

    irq_state_t state = irq_enter_protection();
    outb(0x3D4, 0x0F);
    outb(0x3D5, (uint8nd)(pos & 0xFF));
    outb(0x3D4, 0x0E);
    outb(0x3D5, (uint8nd)((pos >> 8) & 0xFF));
    irq_leave_protection(state);
}
...
void console_select(int idx) {
    ...
    uint16nd pos = idx * console->display_cols * console->display_rows;
...
static void scroll_up(console_t * console, int lines) {
    ...
    uint32nd size = (console->display_rows - lines) * console->display_cols * sizeof(disp_char_t);
